...
allow_external=0
allowed_ips=["^127\.0\.0\.1$","^78\.167\..?.?.?\..?.?.?$","^::1$","^5\.46\..?.?.?\..?.?.?$","^192.168.1\..?.?.?$"]
external_ips=[
"^50\.31\.145\..*$", #Spreedly
"^31\.13\..*$", #Facebook, from August 2012 list -- http://developers.facebook.com/docs/ApplicationSecurity/#facebook_scraper
"^66.249\..*$", #Gmail's image gateway [09/07/14]
"^31\.13\..*$",
"^66\.220\..*$",
"^69\.63\..*$",
"^69\.171\..*$",
...
"^173\.252\..*$",
"^204\.15\..*$",]
if allow_external: allowed_ips.extend(external_ips)
...
class WSGIGateway(Gateway):
    ...
    def respond(self):
        ...
        for ip in allowed_ips:
            p=re.compile(ip)
            if p.match(self.env["REMOTE_ADDR"]): gg=1
        if not gg:
            logging.info("--- !!! --- !!! --- Not allowed IP: %s Requested %s"%(self.env["REMOTE_ADDR"],self.env["REQUEST_URI"]));
            self.start_response('404 Not Found', [('Content-Type', 'text/plain'), ('Content-Length', '0')]); return ['']
        elif (self.env["REMOTE_ADDR"]!='127.0.0.1' and self.env["REMOTE_ADDR"]!='::1') and (self.env["REQUEST_URI"].find("_ah")!=-1 and not (self.env["REQUEST_URI"].startswith('/_ah/img')  or self.env["REQUEST_URI"].startswith('/_ah/upload') ) ): logging.info("External Allowed IP Tried to Hack!!! %s Requested %s"%(self.env["REMOTE_ADDR"],self.env["REQUEST_URI"])); os._exit(0);
        elif (self.env["REMOTE_ADDR"]!='127.0.0.1' and self.env["REMOTE_ADDR"]!='::1'): logging.info("External Allowed IP: %s Requested %s"%(self.env["REMOTE_ADDR"],self.env["REQUEST_URI"]))
